...
class TestDetectOperatorsCli(unittest.TestCase):
    def test_emits_one_record_per_mismatch(self):
        with TemporaryDirectory() as tmp:
            ...
            with redirect_stdout(out):
                exit_code = detect_operators.main(["--rptsched-dir", str(rptsched_dir)])
    ...
    def test_no_mismatch_emits_empty_stream(self):
        with TemporaryDirectory() as tmp:
            rptsched_dir = make_rptsched_dir(tmp, [_schedlist_line("abcd", "SAMEMGR")], [])
            _write_set_file(rptsched_dir, "abcd", "SAMEMGR")
    ...
    def test_does_not_modify_rptsched_dir(self):
        with TemporaryDirectory() as tmp:
            rptsched_dir = make_rptsched_dir(tmp, [_schedlist_line("abcd", "NEWMGR")], [])
            _write_set_file(rptsched_dir, "abcd", "OLDMGR")
            content_before = (rptsched_dir / "abcd.set").read_text()

            with redirect_stdout(io.StringIO()), redirect_stderr(io.StringIO()):
                detect_operators.main(["--rptsched-dir", str(rptsched_dir)])

            self.assertEqual((rptsched_dir / "abcd.set").read_text(), content_before)
